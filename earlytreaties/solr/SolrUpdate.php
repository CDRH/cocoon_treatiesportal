...
class SolrUpdate {
	...
	*/
	...
	function sendUpdate($post_string) {
		$url = "http://".SOLR_META_UPDATE."/update";
		$ch = curl_init();
		...
		$data = curl_exec($ch);
		if (curl_errno($ch)) {
			print "<span style='color:red; font-weight:bold;'>curl_error:</span>".curl_error($ch);
			return false;
		} else {
			curl_close($ch);
			if ( strstr ( $data, '<int name="status">0</int>')) {
				...
				echo "<span style='color:red; font-weight:bold;'>Update Failed</span><pre>$data</pre>";
				return false;
			}
		}

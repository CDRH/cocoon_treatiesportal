<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:dc="http://purl.org/dc/elements/1.1/" version="1.0">
	...
	<add>
		<doc>
			<field name="id"><xsl:value-of select="/TEI.2/@id"/></field>
			<field name="date">
				<xsl:choose>
	...
	<xsl:template match="fileDesc/titleStmt/title[1]">
		<xsl:choose>
			<xsl:when test="//titleStmt/title[@type='main']">
				<field name="title"><xsl:value-of select="normalize-space(//titleStmt/title[@type='main'])"/>&#160;<xsl:value-of select="normalize-space(//titleStmt/title[@type='sub'])"/>
				</field>
			</xsl:when>
			<xsl:otherwise>
				<field name="title"><xsl:value-of select="normalize-space(//titleStmt/title)"/></field>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
				...
				<xsl:for-each select="hand">
					...
					<xsl:choose>
						<xsl:when test="//text/body/div1/sp[@who=$speaker]">
							<field name="author"><xsl:value-of select="@scribe" /></field>
						</xsl:when>
					</xsl:choose>
				</xsl:for-each>
			</xsl:when>
	...
	<xsl:template match="text()">
		<xsl:if test="normalize-space(.) != ''"><field name="text"><xsl:value-of select="normalize-space(.)"/></field></xsl:if>
	</xsl:template>
</xsl:stylesheet>
